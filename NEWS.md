...
- **Modify mode: keyboard exit and screen-reader labels** - Pressing `Escape` while modify mode is active now exits modify mode and returns focus to the Modify toolbar button (the same is done after finishing an inline title edit). Valid elements are announced to screen readers via an `aria-label` of `Click to modify (<type>)`, and warn elements via `Cannot modify: <reason>`; the original `aria-label`/`alt` of each element is restored on exit. Escape is ignored while a heading is being edited inline so the existing revert-on-Escape behavior is preserved.
- **Modify mode: re-activate already-positioned typed elements** (#140) - Elements that have already been positioned and saved (wrapped in `::: {.absolute …}`) are now re-activatable in modify mode for every supported typed inner element: paragraphs, display equations, blockquotes, bullet/ordered lists, tables, figures, display code blocks (`<pre>`), and code chunk outputs / figures (`<div class="cell">`). Previously only `{.absolute}` images and generic `{.absolute}` divs were re-activatable; positioned paragraphs etc. were classified as generic divs and lost their type-specific capabilities (Quill text editing for paragraphs, move-only for equations and tables, etc.). The typed positioned classifier targets the *inner* semantic element, reads position from the wrapping `div.absolute`, and on save rewrites the existing `{.absolute …}` block in place — no nested wrappers. The generic `Positioned divs` classifier skips wrappers claimed by a typed classifier (via `data-typed-positioned-claimed`) to avoid overlapping green rings.
