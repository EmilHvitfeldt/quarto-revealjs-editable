...
- **Modify mode: tables (move only)** (#117) - Pipe tables are now activatable in modify mode. Click a highlighted table to enable drag — resize and rotate are intentionally not provided, since a table's size is determined by its content and column definitions rather than a CSS dimension. Tables are matched to their QMD source by the first (header) row of the table within the slide chunk, with positional fallback when the same header row appears more than once. On save, the entire table block is wrapped in `::: {.absolute left=Xpx top=Ypx}\noriginal table source\n:::`.
