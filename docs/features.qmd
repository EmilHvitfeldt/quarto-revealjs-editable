...
| Modify | Activate plain images, `{.absolute}` images, `{.absolute}` divs, fenced divs, callouts, columns, code blocks, tables, slide titles, or positioned arrows for editing |
...
Elements that can be made editable are highlighted with a **green ring** as soon as you enter modify mode. Click a highlighted element to activate it. Modify mode exits automatically after activation (except for slide titles, which stay in edit mode until you click away or press Enter).
...
Pipe tables are matched to their source by the first (header) row of the table within the slide chunk; if the same header row appears more than once, matching falls back to position. On save, the entire table block is wrapped in `::: {.absolute left=Xpx top=Ypx}\n…\n:::`. Resize and rotate are intentionally not provided — table width is determined by content and column definitions, not a CSS dimension on a wrapper.
...
::: {.callout-note}
Code-generated figures (e.g., plots from R or Python chunks) are not yet supported in modify mode. Non-image chunk outputs — tables, printed text, widgets, OJS — are supported as described above.
:::
