...
Three toggle buttons control how the image fills its box:

| Button | CSS value | Effect |
|--------|-----------|--------|
| Cover | `cover` | Fills the box, cropping if needed |
| Contain | `contain` | Fits fully inside the box |
| Fill | `fill` | Stretches to fill exactly |

Click the active button again to clear the property.

### Flip

Two toggle buttons flip the image:

- **⇆ H** — flips horizontally (`scaleX(-1)`)
- **⇅ V** — flips vertically (`scaleY(-1)`)

Flips compose correctly with any existing rotation.

### Replace Image

Opens a file picker to swap the image source. The new image is embedded as a data URI. Use Save Edits afterwards to persist the change to your QMD file.

::: callout-note
Replace currently embeds images as data URIs. Writing the file path back to the QMD source is not yet supported.
:::

### Reset

Reverts opacity, border radius, object fit, and flips to their defaults without affecting the element's position or size.
