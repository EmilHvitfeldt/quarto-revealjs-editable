...
Plain images are any image inserted directly in a slide without positional attributes. `{.absolute}` divs are elements that have already been positioned by a prior save — they carry `left=`, `top=`, `width=`, and `height=` attributes and are matched back to the source using those values.
