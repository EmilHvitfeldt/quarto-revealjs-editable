...
| Modify | Click any plain image on the slide to make it editable |
...
Not every image needs to be marked with `{.editable}` in advance. Click the **Modify** button in the toolbar to enter modify mode, then click any plain image on the slide to make it editable on the fly.

Images that can be made editable are highlighted with a **green ring** as soon as you enter modify mode. Images that cannot be modified (such as figures generated from code chunks) show no highlight. Click a highlighted image to activate it — it becomes a fully editable element with the same drag, resize, rotate, and image controls as any other editable image.

After saving, the image's position and size are written back to your `.qmd` file.

::: {.callout-note}
Modify mode currently supports images inserted with `![](image.png)` syntax. Code-generated figures (e.g., plots from R or Python chunks) are excluded.
:::
...
When you click an image element, the toolbar switches to an image-specific context panel with controls for styling the selected image.
